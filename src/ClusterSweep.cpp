...
namespace ClusterSweep {
    std::vector<std::vector<Point>> cluster(std::vector<Point>& data, vec3 direction, unsigned precision) {
        ...
        for (double height = top; height >= bottom; height -= 0.75 * step_height) {
            std::vector<Point> points = get_points_in_height_range(point_heights, height, height - step_height);

            int max_cluster_id;
            std::vector<unsigned> clusters = cluster_dbscan(points, 5.0, 6, max_cluster_id);
            ...
            for (int cluster = 0; cluster <= max_cluster_id; cluster++) {
                ...
                std::vector<Point> cluster_points = get_points_from_cluster(points, clusters, cluster);
                int c = -1;

                std::cout << "points in cluster " << cluster << ": " << cluster_points.size() << std::endl;
                ...
                // If points do not overlap with an already existing cluster make a new cluster
                if (c == -1) {
    ...
    bool point_height_pair_cmp(std::pair<Point, double> a, std::pair<Point, double> b) {
        return a.second < b.second;
    }

    std::vector<Point> get_points_in_height_range(std::vector<std::pair<Point, double>>& point_heights, double from, double to) {
    ...
    std::vector<unsigned> cluster_dbscan(std::vector<Point> points, float eps, unsigned min_pts, int &max_cluster_id) {
    ...
    std::vector<Point> get_points_from_cluster(std::vector<Point> points, std::vector<unsigned> clusters, unsigned cluster) {
        std::vector<Point> res;
    ...
    std::optional<unsigned> point_in_cluster(std::vector<std::vector<Point>> clustered_points, Point point) {
        for (int cluster = 0; cluster < clustered_points.size(); cluster++) {
            if (std::find(clustered_points[cluster].begin(), clustered_points[cluster].end(), point) != clustered_points[cluster].end()) {
                return cluster;
            }
        }
